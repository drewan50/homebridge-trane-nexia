...
- `config.schema.json` now declares required fields with a standard JSON Schema object-level `required` array instead of per-field `required: true` booleans (flagged by the Homebridge verification checks).
